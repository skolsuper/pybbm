...
class PybbFeed(PermissionsMixin, Feed):
    feed_type = Atom1Feed
...
class LastPosts(PybbFeed):
    ...
    def items(self, user):
        ids = [p.id for p in self.perms.filter_posts(user, Post.objects.only('id')).order_by('-created', '-id')[:15]]
...
class LastTopics(PybbFeed):
    ...
    def items(self, user):
        return self.perms.filter_topics(user, Topic.objects.all()).select_related('forum').order_by('-created', '-id')[:15]
